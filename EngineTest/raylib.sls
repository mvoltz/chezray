...
(define (make-color r g b a)
  (bitwise-ior r (bitwise-arithmetic-shift-left g 8)
              (bitwise-arithmetic-shift-left b 16)
              (bitwise-arithmetic-shift-left a 24)))

(define draw-rectangle (foreign-procedure "DrawRectangle" (int int int int unsigned-32) void))
...
(define library-filename
  (case (machine-type)
  ;; Windows
  [(a6nt ta6nt i3nt ti3nt) "raylib_shared/raylib.dll"]
  ;; Linux
  [(a6le ta6le i3le ti3le) "raylib_shared/libraylib.so.6.0.0"]
  ;; macOS (Intel and Apple Silicon)
  [(a6osx ta6osx arm64osx tarm64osx) "raylib_shared/libraylib.dylib"]
  ;; Fallback error if it's an unknown system
  [else (error 'raylib "Unsupported machine type" (machine-type))]))
...
;;x   bool WindowShouldClose(void);                               // Check if application should close (KEY_ESCAPE pressed or windows close icon clicked)
...
;;x    bool IsKeyDown(int key);                                // Check if a key is being pressed
...
;;    #define RAYWHITE   (Color){ 245, 245, 245, 255 }   // My own White (raylib logo)(load-shared-object library-filename)
